...
import useS, { state } from "../../src/components/useS";
...
const S = (props: {
  ...
  const SurahAudio = () =>
    showAudio ? (
        ...
        <div id={styles.d34}>
          <div id={styles.d35}>
            <div id={styles.d43}>
              ...
              <h3>
                {surahAudioIndex + 1}. {surahs[surahAudioIndex].title}
              </h3>
            </div>
            <div id={styles.d36}>
              <div
                className={styles.c37}
                onClick={() => {
          ...
          <div id={styles.d42}>
            ...
            <ReactAudioPlayer
              controls
              src={
                surahs[surahAudioIndex].reciters[
                  user.surahAudio.split(" ").join("")
                ]
              }
            />
          </div>
      ...
      <div id={styles.d39} onClick={() => setShowAudio(true)}>
        <h2>Audio ♪</h2>
      </div>
    );

  const BodyFirstPageBanner = () =>
    isFirstPage && (
      <div id={styles.d4}>
        <div id={styles.d5}>
          <h2 id={styles.english}>{cs.title.split(" (")[0]}</h2>
          <h1 id={styles.transliteration}>
            {`${s + 1}. ${cs.title.split(" (")[1].slice(0, -1)}`}
          </h1>
          <h2 id={styles.arabic} className="uthmani">
            {cs.titleAr}
          </h2>
        </div>
      </div>
  ...
  const BodyBanner = () => (
    <div id={styles.d11}>
      ...
      <div className={styles.d4_}>
        <div className={styles.d6_}>
          {isFirstPage ? (
            <h4 style={{ color: "whitesmoke" }}>P{p}</h4>
          ) : (
            <>
              <h3>
                {s + 1}.{" "}
                {cs.title
                  .split(" (")[1]
                  .substring(0, cs.title.split(" (")[1].length - 1)}{" "}
              </h3>
              <h3 id={styles.translation}>{cs.title.split(" (")[0]}</h3>
              <h3 id={styles.p}>P {isFirstPage ? cs.startPage : p} </h3>
            </>
          )}{" "}
        </div>
      </div>
      <div className={styles.d5} onClick={() => setShowPopup(true)}>
        <svg
        ...
        <div id={styles.d30}>
          {verses.map((verse, i) => (
            <>
              ...
              <VerseComponent
                user={user}
                loc={[verse.meta.surah - 1, verse.meta.ayah - 1]}
                verse={verse}
                key={i}
      ...
      <div id={styles.d31}>
        {s !== 0 && (
          ...
          >
            <h1>
              ‹{" "}
              {verses[0].meta.ayah === 1
                ? `${s}. ${ps.title.split(" (")[1].slice(0, -1)}`
                : `P${p - 1}`}
            </h1>
          </div>
        )}
          ...
          >
            <h1>
              {verses[verses.length - 1].meta.ayah === cs.count
                ? `${s + 2}. ${ns.title.split(" (")[1].slice(0, -1)}`
                : `P${p + 1}`}
              {` ›`}
            </h1>
          </div>
      ...
      <Head>
        <title>
          ...
          {`${s + 1}. ${cs.title
            .split(" (")[1]
            .substring(0, cs.title.split(" (")[1].length - 1)
            .replace("'", "&apos;")} (${cs.title.split(" (")[0]})${
            isFirstPage ? "" : ` - P${p}`
          }`}
        </title>
        <meta
          property="og:title"
          content={`${s + 1}. ${cs.title
            .split(" (")[1]
            .substring(0, cs.title.split(" (")[1].length - 1)
            .replace("'", "&apos;")} (${cs.title.split(" (")[0]})${
            isFirstPage ? "" : ` - P${p}`
        ...
        ></meta>
        <meta name="keywords" content={`${cs.title}, ${cs.titleAr}`}></meta>
        <meta
          name="description"
          content={`Verses ${verses[0].meta.ayah}-${
