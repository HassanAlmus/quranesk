...
const returnTitle = (verse, tafseerMap, key) => {
  return `${
    ...
    verse[key][0].split("-")[1] !== verse[key][0].split("-")[2]
      ? `-${+verse[key][0].split("-")[2] + 1}`
      : ""
  }`;
};
...
const VerseComponent = (props: {
    ...
    return (
      <div className={styles.d13} onMouseLeave={() => setS(false)}>
        ...
        <div
          ...
          onClick={() => {
            const audioPart = new Audio(
              `https://raw.githubusercontent.com/HassanAlmus/wbw/main/${(
                props.nav[0] + 1
              )
                .toString()
  ...
  const D26 = () => (
    <div className={styles.d26}>
      {(props.component === "v"
        ? user.translations
        : props.user.surahTranslation
        ? [props.user.surahTranslation]
        : []
      ).map((key: any) => (
        <div
          className={styles.d27}
          id={
            props.highlighted && props.highlighted === key
              ? "highlighted-text"
              : ""
          }
          key={key}
      ...
      {(props.component === "v"
        ...
        return (
          Object.keys(verse).includes(key) &&
          (key !== "khorramdelfa"
            ? ((verse as Verse)[key] as Array<Maybe<Scalars["String"]>>)[0] !==
                null &&
              (key !== "namoonaur" ? (
                <div
                  className={styles.d27}
                ...
                >
                  <div className={styles.d41}>
                    <h1> {returnTitle(verse, tafseerMap, key)} </h1>
                    <div style={{ display: "flex" }}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                    ...
                    {Object.keys(verse).includes(key)
                      ? (verse[key] as NamoonaTopic[]).map((topic, i) => {
                          return (
                            <a
                              key={i}
                              href={`/tafseerenamoona?s=${loc[0]}&v=${loc[1]}&id=${topic.link}`}
                              rel="noreferrer"
                              target="_blank"
                            >
    ...
    <div className={styles.d12}>
      ...
      <h3 className={styles.d28}>
        ...
        <span>
          ...
          {verse.words.map((w, i, arr) => {
            return `${(w as Word).transliteration}${
              i === arr.length - 1 ? "" : " "
            }`;
          })}{" "}
        </span>
      ...
      <div className={styles.d44}>
        ...
        <Link
          href={
            props.component === "v"
              ? `/${verse.meta.surah}?p=${verse.meta.page}`
              : `/${verse.meta.surah}/${verse.meta.ayah}`
          }
        >
  ...
  return (
    <div className="verse">
      ...
      {(props.component === "v"
        ? user.translations.length >= 1 || user.tafseers.length >= 1
        : user.surahTranslation || user.surahTafseer) && <D26 />}{" "}
    </div>
  );
};
