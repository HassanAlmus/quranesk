...
const useV = () => {
  ...
  const [user, setUser] = useState<User>(
    edit(router.query, Cookies.get("user"))
  );
  ...
  const returnCondition = () =>
    loc[0] + 1 === snap.verse.meta.surah && loc[1] + 1 === snap.verse.meta.ayah;
  ...
  const WBWQuery = (t: string) => gql`
    query Query {
      verse(s: ${loc[0].toString()}, v: ${loc[1].toString()}){
        id
        words{
  ...
  query Query {
    cs: surah(s: ${s.toString()}){
      id
      titleAr
      title
      count
    }
    ps: surah(s: ${(s !== 0 ? s - 1 : 0).toString()}){
      id
      count
    }
    verse(s: ${s.toString()}, v: ${v.toString()}) {
     id
     ${(additional !== undefined
       ? [additional as string, ...user.translations, ...user.tafseers]
       : [...user.translations, ...user.tafseers]
     )
       .map((key) => returnKey(key))
       .join("\n")}
     words {
       ${user.wbwtranslation}
       ${user.rasm}
 ...
 `;
    ...
    if (
      ...
      client
        ...
        .then((result) => {
          ...
          if (v === 0 && s > 0) {
            client
              .query(VerseQuery(s - 1, result.data.ps.count - 1))
              .toPromise();
          } else {
  ...
  useEffect(() => {
    if (snap.verse) {
      if (
        !Object.keys(snap.verse.words[0] as Word).includes(user.wbwtranslation)
      ) {
        client
          .query(WBWQuery("w"))
          .toPromise()
              ...
              state.verse = {
                ...
                words: snap.verse.words.map((w: any, i: number) => {
                  ...
                  nw[user.wbwtranslation] =
                    result.data.verse.words[i][user.wbwtranslation];
                  return nw;
                }),
              };
            }
  ...
  const prevVerse = () => {
    if (!(loc[0] === 0 && loc[1] === 0)) {
      client
        .query(
          VerseQuery(
            loc[1] - 1 === 0 ? loc[0] - 1 : loc[0],
            loc[1] - 1 === 0 ? snap.ps.count - 1 : loc[1] - 2
          )
        )
        .toPromise();
      setLoc(
        loc[1] === 0 ? [loc[0] - 1, snap.ps.count - 1] : [loc[0], loc[1] - 1]
      );
    }
  };
  ...
  const nextVerse = () => {
    if (!(loc[0] === 113 && loc[1] === 6)) {
      client
        .query(
          VerseQuery(
            loc[1] + 1 === snap.cs.count ? loc[0] + 1 : loc[0],
            loc[1] + 1 === snap.cs.count ? 0 : loc[1] + 2
          )
        )
        .toPromise();
      setLoc(
        loc[1] + 1 === snap.cs.count ? [loc[0] + 1, 0] : [loc[0], loc[1] + 1]
      );
    }
  };
