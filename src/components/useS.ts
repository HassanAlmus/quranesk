...
import { Audio, Tafseer, TranslationLanguage, User, Word } from "../../utils";
...
const useS = (props) => {
  ...
  const setTranslation = (v: any) =>
    setUser({
      ...user,
      surahTranslation: v,
    });
  const setTafseer = (v: any) =>
    setUser({
      ...user,
      surahTafseer: v,
    });
  ...
  const PageQuery = (_s: null | number, _p: number) => gql`
    query Query{
      page(s: ${_s.toString()}, p: ${_p.toString()}) {
       id
  ...
  useEffect(() => {
    (myRef.current as any).scrollIntoView();
    if (
      window.location.href.split("?p=")[1] !== undefined &&
      Number(window.location.href.split("?p=")[1]) !== cs.startPage
    ) {
      ...
      .then((result) => {
        if (type === "next") {
          ...
          if (result.data.ns && ns.startPage === ns.endPage) {
            client
              .query(PageQuery(s + 1, result.data.ns.startPage))
              .toPromise();
          }
        }
        if (type === "prev") {
          ...
          if (result.data.ps)
            client
              .query(PageQuery(s - 1, result.data.ps.startPage))
              .toPromise();
        }
        setLoadingSurah(false);
      });
    client
      .query(PageQuery(s, (type === "next" ? ns : ps).startPage))
      .toPromise()
      .then((result) => {
        setVerses(result.data.page);
        setLoading(false);
  ...
  const nextPage = () => {
    ...
    if (verses[verses.length - 1].meta.ayah === cs.count) {
      ...
      router.push(`/${s + 2}`, undefined, { shallow: true });
      getSurah("next", s + 1);
      ...
      router.push(`/${s + 1}?p=${p + 1}`, undefined, { shallow: true });
      getPage(p + 1);
      setIsFirstPage(false);
      if (p + 1 === cs.endPage) {
        client.query(SurahQuery("next", s)).toPromise();
  ...
  const prevPage = () => {
    ...
    if (isFirstPage) {
      setLoadingSurah(true);
      router.push(`/${s}`, undefined, { shallow: true });
      getSurah("prev", s - 1);
  ...
  useEffect(() => {
    if (snap.loadedVerses) {
      if (
        !Object.keys(verses[0].words[0] as Word).includes(user.wbwtranslation)
      ) {
